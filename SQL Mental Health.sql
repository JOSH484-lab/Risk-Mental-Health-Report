...
FROM vw_mental_health_analytics;
...
SELECT
ROUND(AVG(CAST( addiction_level AS FLOAT)),2) AS Avg_Stress
FROM vw_mental_health_analytics
--depression rate
SELECT
ROUND(
...
2
) AS Depression_Rate_Percentage
FROM vw_mental_health_analytics
--High risk students (mental_health_risk_score >= 7)
SELECT
COUNT(*) AS High_Risk_Students
FROM vw_mental_health_analytics
WHERE mental_health_risk_score >= 7;
--Percentage of high risk students
SELECT
ROUND(
COUNT(CASE
WHEN mental_health_risk_score >= 7 
THEN 1
END) * 100.0
/
